...
describe("DefaultSkillLocator", () => {
  ...
  it("deduplicates symlink aliases by real path and keeps Pi root precedence", async () => {
    const userPath = "/home/tester/.pi/agent/skills/shared/SKILL.md";
    const globalPath = "/home/tester/.agents/skills/shared/SKILL.md";
    const fs = new MemoryTreeFileSystem(
      [userPath, globalPath],
      new Map([
        [userPath, globalPath],
    ...
    assert.equal(files[0]?.filePath, userPath);
    assert.equal(files[0]?.source.kind, "user");
  });
});
