...
export async function showSkillToggleUi(
  ...
  return ctx.ui.custom<SkillToggleUiResult>(
    (tui, theme, _keybindings, done) => new SkillToggleOverlay(tui, theme, skills, preferences, done),
    {
      overlay: true,
      overlayOptions: {
        anchor: "center",
...
class SkillToggleOverlay {
  ...
  constructor(
    ...
    private readonly preferences: SkillInvocationPreferences,
    private readonly done: (result: SkillToggleUiResult) => void,
  ) {
    for (const skill of skills) this.desired.set(skill.id, preferredMode(skill, preferences));
  }
  ...
  private renderDetails(width: number, height: number): string[] {
    ...
    lines.push("");
    const preference = this.preferences.modes[skillPreferenceKey(skill)];
    ...
    lines.push(`${this.theme.fg("muted", "Policy:")} ${preference ? `managed (${modeLabel(preference)})` : "not managed"}`);
