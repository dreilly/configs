...
export class NodeFileSystem implements FileSystem {
  ...
  async writeFileAtomic(path: string, content: string): Promise<void> {
    // Resolve a symlinked SKILL.md before rename so the atomic replacement does
    // not replace the symlink itself. Directory symlinks are resolved here too.
    let targetPath = path;
    try {
      targetPath = await fs.realpath(path);
    } catch {
      // The caller currently writes existing files, but keep new-file behavior.
    }
